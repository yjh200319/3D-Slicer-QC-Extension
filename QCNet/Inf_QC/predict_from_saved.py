...
os.environ['TF_CPP_MIN_LOG_LEVEL'] = '3'  # 关闭所有 TensorFlow 日志（ERROR及以上级别）
tf.get_logger().setLevel('ERROR')  # 关闭 Keras/TensorFlow 的警告


def predict_from_saved(dwi, mask, bval):
    # ##########
    # 定义目标尺寸
    ...
    # 获取非b0图像的索引
    bval = np.round(bval / 100) * 100
    non_b0_indices = np.where(bval != 0)[0]

    # 统计有多少pass的, 多少个failed
    ...
    for v in non_b0_indices:
        ...
        processed_data = dwi[..., v] / (np.max(dwi[..., v]))
        ...
        # 做出新的修改,即二者调换一下 原本是> threshold的为 Motion Artifact
        # 现在是< threshold为Motion Artifact

        # 先做一个测试,看看如果是婴儿数据的话,改成>效果会不会变化
        if pred > threshold:
            # print('Motion artifact detected')
            pred_label.append(int(1))
        else:
            # print('No artifact detected')
            pred_label.append(int(0))
        ...
        """
    non_b0_indices = non_b0_indices.tolist()
    combined_predict_list = list(zip(non_b0_indices, pred_label))
...
if __name__ == '__main__':
    ...
    # result_list = predict_from_saved(dwi, mask,bval)
    #
    # # 输出 JSON 给父进程
    ...
    bval = np.load(bval_path)

    # 处理数据
    result_list = predict_from_saved(dwi, mask, bval)

    # 输出结果
    print(json.dumps({"result": result_list}), flush=True)
